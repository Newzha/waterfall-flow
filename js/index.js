...
*/
  ...
  let bindHTML = () => {
    ...
    // 当前这一组数据绑定完成后，让isRun=false，代表运行完成了
    isRun = false;
  };
  bindHTML();
  ...
  $(window).on('scroll', () => {
    ...
    // 卷去的高度 大于 真实高度-屏幕高度：距离底部还有100PX，便开始加载更多的数据
    if ((scrollT + 100) >= (pageH - winH)) {
      // 隐性问题：认为操作滚动，这个在同一个操作内会被触发N次，
      // 也就是同一个时间段，获取数据会被执行N次，此时我们需要做“重复操作限定”
      if (isRun) { return };
      isRun = true; // 当前开始进行新一轮的加载数据
